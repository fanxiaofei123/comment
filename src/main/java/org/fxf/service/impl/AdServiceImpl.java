...
public class AdServiceImpl implements AdService{
	...
	public List<AdDto> searchByPage(AdDto adDto) {
		...
		for (Ad ad : adList) {
			AdDto aDtoTemp = new AdDto();
			result.add(aDtoTemp);
			aDtoTemp.setImg(adImageUrl+ad.getImgFileName());
			BeanUtils.copyProperties(ad, aDtoTemp);
		}
	...
	public boolean modify(AdDto adDto) {
		...
		if (adDto.getImgFile() != null && adDto.getImgFile().getSize() > 0) {
			//删除原来的图片
			FileUtil.delete(adImageSavePath + adDao.selectById(ad.getId()).getImgFileName());
